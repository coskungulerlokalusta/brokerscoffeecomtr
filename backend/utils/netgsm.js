...
async function sendSms(phone, message) {
  const creds = await integrations.getProviderCredentials('netgsm');
  if (!creds || !creds.enabled) {
    throw new Error('NetGSM entegrasyonu aktif değil');
  }
  if (!creds.username || !creds.password || !creds.header) {
    throw new Error('NetGSM bilgileri eksik (kullanıcı adı/şifre/başlık)');
  }
